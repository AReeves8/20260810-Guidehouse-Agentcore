...
from ticket_api.tickets import store
...
def get_tickets():
    ...
    if priority is None:
        return list_envelope(store.list_tickets())

    # return matching tickets
    return list_envelope(store.find_tickets_by_priority(priority))
...
def get_ticket_by_id(ticket_id):
    ...
    """
    ticket = store.find_ticket_by_id(ticket_id)
...
def create_new_ticket():
    ...
    return single_envelope(store.create_ticket(body)), 201        # setting status code as 201 - CREATED
...
def update_existing_ticket(ticket_id):
    ...
    body = request.get_json(silent=True) or {}     # if there's no request body, set the value to an empty object
    return single_envelope(store.update_ticket(ticket_id, body)), 200     # if you want to return nothing on updates, return a 204 instead of 200

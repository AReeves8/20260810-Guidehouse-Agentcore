...
from ticket_api.health.routes import health_bp
from ticket_api.tickets.routes import tickets_bp
...
def create_app():
    ...
    """
        in cli: 
            db init - setups migrations with Alembic for project (only needed once)
            db migrate - identifies differences between app models and actual db tables. notes them in file under /versions
            db upgrade - creates the changes in the most recent migration version
    """
    migrate.init_app(app, db)                                               # helps manage tables in db
